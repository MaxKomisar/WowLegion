...
class modDphpBB3LastTopicsHelper
{
	...
	function getList($params)
	{
		...
		// WHERE conditions for Locked and Sticky, Announce or Global topics
		$where_status_type = '';
		if (!$params->get('show_locked', true)) {
			$where_status_type .= '	AND topic_status <> 1';
		}
		if (!$params->get('show_normal', true)) {
			$where_status_type .= '	AND topic_type <> 0';
		}
		if (!$params->get('show_sticky', true)) {
			$where_status_type .= '	AND topic_type <> 1';
		}
		if (!$params->get('show_announce', true)) {
			$where_status_type .= '	AND topic_type <> 2';
		}
		if (!$params->get('show_global', true)) {
			$where_status_type .= '	AND topic_type <> 3';
		}
		// Topics
		$topics_list = array();
